...
class Spreadsheet_Excel_Reader {
	...
	// =========================================
	function import_prepare($row_numbers=false,$col_letters=false,$sheet=0,$table_class='excel') {
		for($row=1;$row<=$this->rowcount($sheet);$row++) {
			echo ($row-1)."\t";
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				if(($col==1)||($col==3)){
					$cellval = $this->val($row,$col,$sheet);
					echo $cellval;
					echo "\t";
			}
			}
...
//		return $out;
	...
	function import($row_numbers=false,$col_letters=false,$sheet=4,$table_class='excel') {
		for($row=2;$row<=$this->rowcount($sheet);$row++) {
			$temp = array();$head = array();
			for($col=1;$col<=$this->colcount($sheet);$col++) {
				$headval = $this->val(1,$col,$sheet);
				array_push($head,$headval);
				$cellval = $this->val($row,$col,$sheet);
				array_push($temp,$cellval);
			}
			echo "insert into xxx20211214(".implode(",",$head).") <br />";
			echo "values ('".implode("','",$temp)."');";
			echo "<br />";
		}
//		return $out;
	}
